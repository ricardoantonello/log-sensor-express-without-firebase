...
* It pushes the provided listener to our array of listeners (either the temperature
* listener array, or the humidity listener array, depending on the "type" provided)
*/
...
const unsubscribe =(removedListener, type)=>{
    ...
    * the basis of whether they are the same as the listener providedThose that are the same
    * (which would be only one) are removed
    */
    listeners[type]= listeners[type].filter(listener => listener !== removedListener)
}

/**
* The notify function is used by the cache when it receives an update from the sensor
* It takes a value and a type as an argument, and notifies all the listeners of that
* type that a new value has been received
*/
